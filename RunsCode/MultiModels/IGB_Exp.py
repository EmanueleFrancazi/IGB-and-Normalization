...
@torch.no_grad()   #we just need output stas here, not gradient; so we add this decoration to avoid its computation
def SingleInitStats(train_loader, par):
    """
...
IGB_Mode='On' #can be 'On' or 'Off' changing the activation function or elements used in the architecture
...
Architecture = 'MLP-mixer' #can be 'MLP' or 'MLP-mixer' or 'BaseCNN' or 'ViT' or 'ResNet' 'MLP_mixer2'
...
NormMode = 'OFF' #4 possible modes: 'Shift'==add an offset to center the activated nodes, 'OFF'==No normalization, 'InitShift' == shift computed on the first batch and same statistics used for all the train steps 'BN' classic batch norm
...
if TrainMode=='OFF':
    ...
    with open('./ClassesMaxf.txt', "a") as f:
...
#Training the model
    ...
    Sigma_Single_Stats = SingleInitStats(train_dl, params)
